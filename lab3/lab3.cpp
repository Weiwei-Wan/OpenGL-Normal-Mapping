...
using namespace std;
GLuint SkyBoxID, LightMapID, NormalMapID, BumpMapID;
GLuint brickWallMap, brickWallNormalMap, brick2Map, brick2NormalMap, basMap, basBumpMap;
...
GLfloat light_pos_x = 5.0f;
GLfloat light_pos_y = 1.0f;
...
void displayNormalObject(GLuint& ID, GLuint& diffuse_map, GLuint& normal_map, glm::vec3 pos, ModelData mesh_data, GLuint type, float scale) {
	glUseProgram(ID);
	...
	glUniform3f(glGetUniformLocation(ID, "viewPos"), camera_pos_x, camera_pos_y, camera_pos_z);
	glUniform3f(glGetUniformLocation(ID, "lightPos"), light_pos_x, light_pos_y, light_pos_z);
...
void display() {
	...
	if (MODE == 1 || MODE == 7) {
		// normal map
		displayNormalObject(NormalMapID, brickWallMap, brickWallNormalMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_teapot, 1, 0.2f);
		// lighting map
		displayNormalObject(LightMapID, brickWallMap, brickWallNormalMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_teapot, 1, 0.2f);
	}
	else if (MODE == 2) {
		displayNormalObject(NormalMapID, brick2Map, brick2NormalMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_teapot, 1, 0.2f);
		displayNormalObject(LightMapID, brick2Map, brick2NormalMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_teapot, 1, 0.2f);
	}
	else if (MODE == 3) {
		displayNormalObject(NormalMapID, brickWallMap, brickWallNormalMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_bunny, 2, 1.0f);
		displayNormalObject(LightMapID, brickWallMap, brickWallNormalMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_bunny, 2, 1.0f);
	}
	else if (MODE == 4) {
		displayNormalObject(NormalMapID, brick2Map, brick2NormalMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_bunny, 2, 1.0f);
		displayNormalObject(LightMapID, brick2Map, brick2NormalMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_bunny, 2, 1.0f);
	}
	else if (MODE == 5) {
		displayNormalObject(NormalMapID, basMap, basBumpMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_teapot, 1, 0.2f);
		displayNormalObject(LightMapID, basMap, basBumpMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_teapot, 1, 0.2f);
	}
	else if (MODE == 6) {
		displayNormalObject(NormalMapID, basMap, basBumpMap, glm::vec3(0.0f, 0.0f, 3.5f), mesh_bunny, 2, 1.0f);
		displayNormalObject(LightMapID, basMap, basBumpMap, glm::vec3(0.0f, 0.0f, -3.5f), mesh_bunny, 2, 1.0f);
	}

	if (MODE < 5) {
		drawText("Normal mapping", 3, glm::vec3(5.0f, 4.0f, 0.0f));
		drawText("Lighting map", 3, glm::vec3(16.0f, 4.0f, 0.0f));
	}
	else if (MODE < 7) {
		drawText("Bump mapping", 3, glm::vec3(5.0f, 4.0f, 0.0f));
		drawText("Lighting map", 3, glm::vec3(16.0f, 4.0f, 0.0f));
	}
...
void init()
{
	...
	brickWallNormalMap = loadTexture("./textures/brickwall_normal.jpg");
	brick2Map = loadTexture("./textures/bricks2.jpg");
	brick2NormalMap = loadTexture("./textures/bricks2_normal.jpg");
	basMap = loadTexture("./textures/pic.jpg");
	basBumpMap = loadTexture("./textures/bump.jpg");
	SkyBoxID = CompileShaders("./shaders/skyboxVertexShader.txt", "./shaders/skyboxFragmentShader.txt");
	LightMapID = CompileShaders("./shaders/lightMapVertexShader.txt", "./shaders/lightMapFragmentShader.txt");
	NormalMapID = CompileShaders("./shaders/normalMapVertexShader.txt", "./shaders/normalMapFragmentShader.txt");
}
